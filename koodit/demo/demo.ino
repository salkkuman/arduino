...
#include "Apple.h"
#include "Wormpart.h"
//to possibly move the letters
int placeOfChar=0;
//decides which letter is on
int letter=0;
//decide which mode is run
int mode=3;
//index to run for colours
int i=0;
//index to run for pong to terminate after while
int iter=0;
...
void loop()
{
  ...
  //colour using modes (all on||letters)
  if(mode==1 || mode==2){
    for(; i<1157561; i+=3) {
  ...
  if(mode==3){
    ...
    if(currentMillis - previousMillis > interval) {
      ...
      boolean alive = true;
      int iterr=0;
      //check if the worm is dead in direction until it can move alive or is determined to die
      while(iterr<7){
